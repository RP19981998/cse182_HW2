...
def gram_matrix(features, normalize=True):
    ...
    geatures.view(N,C,H*W)
    #展平高度和宽度
    
    gram=features.matul(features.permute(0,2,1))
    #对特征图进行维度置换，将通道维度移到最后，[N,H*W,C]
    #NxCxH*W matul NxH*WxC => NxCxC
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    #                               YOUR CODE HERE                               #
    ##############################################################################
    stylr_loss=torch.tensor(0.).type(dtype)
    for i in range(len(style_layer)):
        g=gram_matrix(feats[style_layers[i]])
        stype_loss+=style_weights[i]*torch.sum(g-style_targets[i]**2)
    return style_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
